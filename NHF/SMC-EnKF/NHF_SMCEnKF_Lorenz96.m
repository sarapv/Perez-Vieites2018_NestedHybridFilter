function [ Output_SMCEnKF ] = NHF_SMCEnKF_Lorenz96( nosc , N, gap, t_final, iter )
%
%   [Output_SMCEnKF] = NHF_SMCEnKF_Lorenz96( nosc, M, gap, t_final, iter )
%
% Inputs:
...
%
% Outputs:
%   Output_SMCEnKF   : structure that contains 
...
% Particle filter and ensemble parameters
M_ensemble = nosc;
F_range = [2 30];
A_range = [0 0.2; 0 0.2];
FA_range = [F_range; A_range];
s2M = [20 20 1/5 20 0.04 0.04]./(N^1.5); %[20 20 1/5 20 0.04 0.04]./(M^1.5); % [F C B H A1 A2]
...
[FAest, Xest, FAparticles, MSEx, MSE_ansatz, xref]= SMCEnKF_Lorenz96(s2x,s2z,s2y,h,NT,Tobs,K,FA_range,N,M_ensemble,nosc,F,C,B,H,fps,s2M([1 5]));

ttotal = etime(clock,t0)/60;

fprintf(1, 'time to finish = %7.3f min\n',ttotal);

Output_SMCEnKF = struct('FAest',FAest,'Xest',Xest,'FAparticles',FAparticles,'MSEx',MSEx, 'MSE_ansatz', MSE_ansatz,'xref',xref);  
fprintf(1,'SMC+EnKF (F unk., ansatz): MSEx = %7.3f \n', mean(MSEx(1+Tobs:Tobs:NT)));
fprintf(1,'\n ------------------------------------------------------------- \n \n');

% Save data
etiq_save = sprintf('data/SMCEnKF_FA_nosc%d_N%d_M%d_Tobs%d_iter%d.mat', nosc, N, M_ensemble, Tobs, iter);
save(etiq_save);
