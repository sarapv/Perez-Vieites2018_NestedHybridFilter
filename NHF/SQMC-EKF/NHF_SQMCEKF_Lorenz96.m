function [ Output_SQMCEKF ] = NHF_SQMCEKF_Lorenz96( nosc , M, gap, t_final, iter )
%
%   [Output_SQMCEKF] = NHF_SQMCEKF_Lorenz96( nosc, M, gap, t_final, iter )
%
% Inputs:
...
s2M = [20 20 1/5 20 0.04 0.04]./(M^1.5); % jittering constant for each parameter: [F C B H A1 A2]
...
[FAest_sqmc, Xest_sqmc, FAparticles_sqmc, MSEx_sqmc, MSE_ansatz, xref] = SQMCEKF_L96(s2x,s2z,s2y,h,NT,Tobs,K,FA_range,s2M([1 5]),M,F,C,B,H,fps,nosc);
       
ttotal = etime(clock,t0)/60;

fprintf(1, 'time to finish = %7.3f min\n',ttotal);       
fprintf(1,'SQMC-EKF (F unk., ansatz): MSEx = %7.3f\n', mean(MSEx_sqmc(1+Tobs:Tobs:NT)));
fprintf(1,'\n --------------------------------------------------------------- \n \n');

% Save data
Output_SQMCEKF = struct('FAest',FAest_sqmc,'Xest',Xest_sqmc,'FAparticles',FAparticles_sqmc,'MSEx',MSEx_sqmc, 'MSE_ansatz', MSE_ansatz,'xref',xref);  
etiq_save = sprintf('data/SQMCEKF_FA_nosc%d_M%d_Tobs%d_iter%d.mat', nosc, M, Tobs, iter);
save(etiq_save);
