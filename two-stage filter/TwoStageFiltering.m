...
[MSE, Aest, Fest, Xest, x_ref]= SF_RK4(s2x,s2y,s2z,h,NT,Tobs,K,FA_range,N,M,F,C,B,H,nosc,fps,s2M([1 5])); 

ttotal = etime(clock,t0)/60;
fprintf(1, 'time to finish = %7.3f min\n',ttotal);

fprintf(1,'MC+EnKF (F unk., ansatz): MSE = %7.3f \n', mean(MSE(1+Tobs:Tobs:NT)));
fprintf(1,'\n ------------------------------------------------------------------------------------------------- \n \n');

Output_2SF = struct('Aest',Aest,'Fest',Fest,'Xest',Xest,'MSE',MSE);
