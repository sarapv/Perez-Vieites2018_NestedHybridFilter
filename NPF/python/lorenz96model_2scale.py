...
def lorenz96_2scale(dx, fps, h, t_final, integration_method):
    """
    Simulates a Lorenz96 model :true_param integration_method: "euler" or "runge-kutta4" are available to integrate
    the SDEs of Lorenz96 :true_param dx: dimension of the state x, e.g., dx = 20 :true_param fps: number of fast
    variables per each slow variables x (i.e., dimension of z = fps * dx), e.g., fps = 10 :true_param h: step-size
    for the integration of the SDEs (in natural units), e.g., h = 2e-4 :true_param t_final: length of the simulation
    in natural time units (i.e., t_final/h discrete time steps), e.g., t_final = 10
    ...
    if integration_method == "euler":
        lorenz96_1timestep = partial(euler_lorenz96_1timestep, dx=dx, dz=dz, fps=fps, h=h, param=param,
                                     noise_var=noise_var)
    elif integration_method == "runge-kutta4":
        lorenz96_1timestep = partial(runge_kutta4_lorenz96_1timestep, dx=dx, dz=dz, fps=fps, h=h, param=param,
                                     noise_var=noise_var)
    else:
    ...
    while not ok:
        for t in np.arange(1, T):
            x[:, t], z[:, t] = lorenz96_1timestep(x_old=x[:, t - 1], z_old=z[:, t - 1])
